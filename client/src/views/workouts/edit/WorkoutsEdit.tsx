...
import SearchExerciseCreate from "../create/components/SearchExerciseCreate";
...
function WorkoutsCreate() {
  ...
  const methods = useForm<IWorkoutFormTitles>({
    defaultValues: {
      title: workout?.name || "",
      monday: workout?.days?.monday?.name || "",
      tuesday: workout?.days?.tuesday?.name || "",
      wednesday: workout?.days?.wednesday?.name || "",
      thursday: workout?.days?.thursday?.name || "",
      friday: workout?.days?.friday?.name || "",
      saturday: workout?.days?.saturday?.name || "",
      sunday: workout?.days?.sunday?.name || "",
    },
  });
  ...
  useEffect(() => {
    ...
    axios
      .get(`/workoutroutine/getWorkoutById/${params.id}`)
      .then((data) => {
        setWorkout({
          ...workout,
        ...
        setValue("title", data.data.data.name);
        setValue("title", data.data.data.days?.monday?.name || "");
        setValue("title", data.data.data.days?.tuesday?.name || "");
        setValue("title", data.data.data.days?.wednesday?.name || "");
        setValue("title", data.data.data.days?.thursday?.name || "");
        setValue("title", data.data.data.days?.friday?.name || "");
        setValue("title", data.data.data.days?.saturday?.name || "");
        setValue("title", data.data.data.days?.sunday?.name || "");
      })
      .catch((error) => {
        if (error.response?.data?.error) {
          ...
          setErr("Error retrieving the workout, try again!");
        }
      })
      .finally(() => {
        // setLoading(false);
  ...
  const saveWorkout = () => {
    ...
    axios
      .patch(`/workoutroutine/update/${params.id}`, {
        // userId: user._id,
        name: workoutName,
  ...
  return (
    <>
      <div>
        <p className="text-2xl font-bold">Edit Your Workout</p>
        {/* <p className="text-lg">
